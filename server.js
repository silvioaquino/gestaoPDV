...
router.get('/pdv',function(req,res){
...
async function criarTabelas() {
  ...
  try {
    ...
    // Tabela vendas_manuais - COM UUID EXPLÍCITO
await client.query(`
...
// Criar índice para vendas manuais
await client.query(`
...
// Endpoint para fechar caixa (ATUALIZADO para incluir vendas manuais)
app.post('/caixa/fechar', async (req, res) => {
  ...
  try {
    ...
    // Calcular totais (incluindo vendas manuais)
    ...
    // Calcular saldo final (APENAS vendas em dinheiro do sistema + manuais)
    ...
    console.log('✅ Caixa fechado com sucesso. Vendas manuais incluídas:', vendasManuaisResult.rows.length);
...
app.post('/vendas/manuais', async (req, res) => {
  ...
  try {
    ...
    console.log('✅ Venda manual salva com sucesso:', result.rows[0].id);
...
app.delete('/vendas/manuais/:id', async (req, res) => {
  ...
  try {
    ...
    console.log('🗑️ Venda manual excluída:', id);
...
// Endpoint para buscar vendas manuais por data (para consulta)
app.get('/vendas/manuais/data/:data', async (req, res) => {
  try {
    const { data } = req.params;
    
    const result = await pool.query(
      `SELECT vm.*, ca.data_abertura 
    ...
    console.error('Erro ao buscar vendas manuais por data:', error);
    res.status(500).json({
      success: false,
      message: 'Erro ao buscar vendas manuais por data'
    });
  }
});


// Endpoint para buscar vendas por data (incluindo manuais)
app.get('/vendas/data/:data', async (req, res) => {
  try {
    const { data } = req.params;
    
    // Buscar vendas do sistema
